...
算术强度 `AI = 1 FLOP / 12B`（1 次加法 ↔ 读 8B + 写 4B）。A100 的平衡点（Ridge Point）约 `60 FLOP/B`（fp32）。`AI ≪ Ridge Point` → **memory-bound**，性能上限由 HBM 带宽决定，而非算力。
...
// 编译命令: nvcc -O3 -arch=sm_80 matrix_addition_float4.cu -o matadd
// 运行:     ./matadd 4096 4096
...
```bash
nvcc -O3 -arch=sm_80 matrix_addition_float4.cu -o matadd
./matadd 4096 4096
```
...
A100 的 HBM 峰值带宽约 `1550 GB/s`，本题达到 `~30%`。看起来不高，但 `cudaEvent` 计时含 launch 开销，且 `effective bandwidth` 按 `3×bytes`（读+写）算偏保守。用 `ncu` 单独测 kernel 可见 `dram__throughput` 更高。
...
3. **kernel 融合**：若 Matrix Addition 是某流水线的中间步骤（如 `C = (A+B) * scale`），可融合成单 kernel 避免 `C` 的中间写读。类似 [Dot Product #17](../week2/day1/leetgpu-dot-product-solution.md) 的融合思想。

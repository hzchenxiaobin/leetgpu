...
// 编译命令: nvcc -O3 -arch=sm_80 relu_fmaxf.cu -o relu
// 运行:     ./relu 25000000
...
```bash
nvcc -O3 -arch=sm_80 relu_fmaxf.cu -o relu
./relu 25000000
```
...
```bash
# 分别编译两个版本
nvcc -O3 -arch=sm_80 -DUSE_IFELSE relu_fmaxf.cu -o relu_ifelse
nvcc -O3 -arch=sm_80                   relu_fmaxf.cu -o relu_fmaxf
...
| **瓶颈类型** | **memory-bound**：算术强度远低于 GPU 平衡点（A100 约 60 FLOP/B），完全被 HBM 带宽限制。比 Vector Addition（0.083 FLOP/B）更彻底 |

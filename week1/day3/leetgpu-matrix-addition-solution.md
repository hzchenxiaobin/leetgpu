...
> 💡 Matrix Addition 是**纯 element-wise**——每个输出 `C[i]` 只依赖 `A[i]` 和 `B[i]`，无邻域复用。因此**不需要 shared memory**（与 [Matrix Transpose #3](../../leetgpu/week1/day4/leetgpu-matrix-transpose-solution.md) 的 tiling、[Matrix Multiplication #2](../../leetgpu/week1/day6/leetgpu-matrix-multiplication-solution.md) 的 tile 复用截然不同）。优化全在 global memory 访问效率上。

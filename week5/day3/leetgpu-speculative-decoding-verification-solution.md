...
> 💡 这道题是 [Week5 Day3](../../aiinfra/week5/day3/README.md) 讲的 **vLLM Worker 执行的验证 kernel**。Speculative Decoding 是推理系统的调度层优化——Scheduler 编排"draft 批量生成 + target 批量验证"迭代，Worker 跑验证 kernel。本题就是那个 kernel：对 B 个序列并行做 accept/reject + resample。B 序列的并行度正是 Continuous Batching 拼出来的 batch。

...
朴素做法：先算 `S = QK^T/√d`（M×M）写回 HBM，再用 mask kernel 置 `-∞`，再 softmax，再乘 V。**致命问题**：`M=5000` 时 `S` 占 `5000²×4B = 100MB`，且 mask 是单独一趟 kernel。与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 同理——应融合成一个 kernel，用 online softmax 不物化 `S`。
...
2. **online softmax 三公式**（与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 一致）：遍历 key 时增量更新 `m/l/o`，`S=QK^T` 和 `P=softmax(S)` 永不落 HBM。

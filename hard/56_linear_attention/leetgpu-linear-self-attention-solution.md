...
5. **warp shuffle 树形归约**（$z$ kernel）：`__shfl_down_sync` 两级归约（warp 内 + 跨 warp 共享内存），是 [Day 4 Reduction](../../medium/4_reduction/leetgpu-reduction-solution.md) 模板的直接复用。
...
6. **Tensor Core（FP16）**：$S$ 和 num 是 GEMM，可用 `wmma`/`mma` 做 FP16 输入 + FP32 累加，算力提升数倍（参考 [GEMM 题解](../../medium/22_gemm/leetgpu-gemm-solution.md) 的 WMMA 用法）。

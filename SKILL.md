...
所有 LeetGPU 题解**写在 `leetgpu/` 目录下**，采用**扁平结构**（不按周/日分层）：
...
2. **文件命名**：`leetgpu-<slug>-solution.md`，其中 `<slug>` 是 LeetGPU 平台的题目 URL slug（如 `vector-add`、`prefix-sum`）。
3. **图片目录**：`leetgpu/images/`，所有题解共享。图片在题解中用 `images/xxx.svg` 相对路径引用。
4. **扁平结构**：不按 week/day 分层，因为 LeetGPU 题目数量有限（~13 道），扁平更清晰。
...
- 图片引用用相对路径：`![<中文alt>](images/<filename>.svg)`。
...
- `build.py` 扫描 `leetgpu/` 下所有 `leetgpu-*.md` 文件（非递归）。
...
- [ ] 题解位于 `leetgpu/leetgpu-<slug>-solution.md`
- [ ] 一级标题 `# LeetGPU <题目名> 题解`

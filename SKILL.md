...
| **题目不重复** | 同一道题在整个题解系列中只出现一次，选题前先查下表状态列（✅ 为已完成）和 `leetgpu/` 已有文件 |
...
下表为各概念分组的题目，选题时按概念覆盖。已完成的题目标注 ✅。

#### 入门：Memory-Bound 基础（grid-stride loop、coalesced access）

| slug | 题目 | 难度 | 核心概念 | 状态 |
|------|------|------|----------|------|
| vector-add | Vector Add | 简单 | grid-stride loop、coalesced 读写 | ✅ |
| relu | ReLU | 简单 | 逐元素 kernel、分支开销 | ✅ |
| matrix-addition | Matrix Addition | 简单 | 2D grid、float4 向量化访存 | ✅ |

#### 进阶：Shared Memory 与 Tiling

| slug | 题目 | 难度 | 核心概念 | 状态 |
|------|------|------|----------|------|
| matrix-transpose | Matrix Transpose | 中等 | shared memory tiling、bank conflict padding | ✅ |
| reduction | Reduction | 中等 | 树形归约、warp shuffle `__shfl_down_sync`、block 两级归约 | ✅ |
| histogram | Histogram | 中等 | shared memory 直方图、atomic 冲突 | ✅ |
| convolution | Convolution | 中等 | shared memory halo、常数内存 | ✅ |

#### 高阶：Warp 级原语与并行模式

| slug | 题目 | 难度 | 核心概念 | 状态 |
|------|------|------|----------|------|
| prefix-sum | Prefix Sum | 中等 | warp scan `__shfl_up_sync`、三阶段分块 scan | ✅ |
| softmax | Softmax | 中等 | 三遍 kernel（max→sum→scale）、数值稳定性 | ✅ |
| argmax | Argmax | 中等 | warp 归约带索引、tie-breaking | ✅ |

#### 综合：Compute-Bound 与融合 Kernel

| slug | 题目 | 难度 | 核心概念 | 状态 |
|------|------|------|----------|------|
| gemm | GEMM | 困难 | tiling、register blocking、双缓冲 | ✅ |
| matrix-multiplication | Matrix Multiplication | 困难 | tiled matmul、bank conflict 分析、register tiling | ✅ |
| attention | Attention | 困难 | fused softmax+matmul、FlashAttention 思想 | ✅ |
...
所有 LeetGPU 题解**写在 `leetgpu/` 下**，按"周 → 日"两级组织，结构参照 `leetcode/daily/`：
...
│   ├── day3/
│   │   └── leetgpu-matrix-addition-solution.md
│   ├── day4/
│   │   └── leetgpu-matrix-transpose-solution.md
│   ├── day5/
│   │   └── leetgpu-reduction-solution.md
│   ├── day6/
│   │   └── leetgpu-matrix-multiplication-solution.md
│   └── day7/
│       └── leetgpu-attention-solution.md
├── week2/
│   ├── day1/
...
2. **周级目录**：`weekN`（`week1`、`week2`、`week3` …），编号从 1 开始连续，与 `leetcode/daily/weekN` 对齐。
3. **日级目录**：每周 `day1` 到 `day7` 共 7 个目录，编号从 1 开始连续。
4. **题解文件名**：`leetgpu-<slug>-solution.md`，其中 `<slug>` 是 LeetGPU 平台的题目 URL slug（如 `vector-add`、`prefix-sum`）。文件放在对应的 `weekN/dayM/` 下。
5. **图片目录**：`leetgpu/images/`，所有题解共享（不按 week/day 分散）。图片在题解中用 `../../images/xxx.svg` 相对路径引用（从 `weekN/dayM/` 出发）。
6. **周/日与每日教程对齐**：`leetgpu/weekN/dayM/` 的题对应 `aiinfra/weekN/dayM/` 每日教程的"任务 4" LeetGPU 在线题目，保持周/日编号一致。

## 3. 题解文档结构

每篇题解 `.md` 遵循固定 **6 段结构**（参考 `leetgpu-prefix-sum-solution.md`、`leetgpu-reduction-solution.md`）：
...
- 图片引用用相对路径：`![<中文alt>](../../images/<filename>.svg)`（从 `weekN/dayM/` 出发，`../../images/` 解析到 `leetgpu/images/`）。
...
- `build.py` **递归扫描** `leetgpu/` 下所有 `leetgpu-*.md` 文件（含 `weekN/dayM/` 子目录）。
...
> ⚠️ **注意 `build.py` 需支持递归扫描**：当前 `leetgpu/website/build.py` 用 `leetgpu_dir.iterdir()`（非递归）。迁移到 `weekN/dayM/` 结构后，需改为 `leetgpu_dir.rglob("leetgpu-*.md")` 递归查找，并相应调整图片路径重写逻辑（`../../images/` → `./images/`）。参考 `leetcode/website/build.py` 的 `rglob` 实现。
...
- [ ] 题解位于 `leetgpu/weekN/dayM/leetgpu-<slug>-solution.md`
- [ ] 一级标题 `# LeetGPU <题目名> 题解`

...
> 💡 这是最简"多通道→单通道"elementwise kernel——每像素读 3 个 float、做 2 次乘加、写 1 个 float。与 [#7 Color Inversion](../../easy/7_color_inversion/leetgpu-color-inversion-solution.md) 同属图像 elementwise，区别是 Color Inversion 是 3→3 通道（保持通道数），本题是 3→1 通道（通道融合）。核心 CUDA 模板与 [#21 ReLU](../../easy/21_relu/leetgpu-relu-solution.md) 完全一致：一元素一线程 + grid-stride loop。

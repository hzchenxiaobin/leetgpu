...
由于矩阵在显存中是**行主序连续存储**的，`M×N` 矩阵等价于长度 `M*N` 的一维 `float` 数组。因此**无需 2D block / tiling**——直接用 **1D grid-stride loop**（[Day 1 Vector Addition](../../easy/1_vector_add/leetgpu-vector-addition-solution.md) 同款）把线程映射到线性下标，自然保证 warp 内地址连续，coalesced 自动满足。
...
> 💡 为什么不用 2D block（`TILE×TILE`）？因为拷贝**无邻域复用**，2D 索引只增加 `row*N+col` 的乘法开销，却换不到 coalescing 收益（1D 已完美 coalesced）。2D block 在 [Matrix Transpose #3](../../easy/3_matrix_transpose/leetgpu-matrix-transpose-solution.md) 那种「读写维度错位」场景才有意义，纯拷贝用 1D 最简洁。
...
> 💡 纯拷贝是「**读一次写一次、用完即弃**」的算子，没有 shared memory 的用武之地。这与 [Matrix Addition #8](../../easy/8_matrix_addition/leetgpu-matrix-addition-solution.md) 一致，而与 tiling 类 kernel（Transpose、GEMM）截然相反。优化全部集中在 **global memory 访问效率**上。

...
把 `M×N` 矩阵视为一维 `float` 数组（行主序连续），用 **1D grid-stride loop**（[Day 1 Vector Addition](../../easy/1_vector_add/leetgpu-vector-addition-solution.md) 的同款模式）映射线程到元素。在此基础上叠加 `float4` **向量化**：每 thread 一次处理 4 个 float（128-bit），把 4 条 `load` 合并为 1 条 `float4` 加载。
...
> 💡 Matrix Addition 是**纯 element-wise**——每个输出 `C[i]` 只依赖 `A[i]` 和 `B[i]`，无邻域复用。因此**不需要 shared memory**（与 [Matrix Transpose #3](../../easy/3_matrix_transpose/leetgpu-matrix-transpose-solution.md) 的 tiling、[Matrix Multiplication #2](../../easy/2_matrix_multiplication/leetgpu-matrix-multiplication-solution.md) 的 tile 复用截然不同）。优化全在 global memory 访问效率上。

...
> 💡 这道题是 [2D Convolution（#10）](../../medium/10_2d_convolution/leetgpu-2d-convolution-solution.md) 的"max reduction"变体。两者都是滑窗 stencil：每个输出对应输入中一个 `K×K` 邻域。差异在于：卷积是**加权求和**（reduce by sum），池化是**取最大值**（reduce by max）。由于 `K=3` 小且 max 操作不可分离（不像 Gaussian blur 可行列分离），朴素滑窗 + 直接 global 访存是标准做法；边界 padding 用 `-∞` 跳过即可。

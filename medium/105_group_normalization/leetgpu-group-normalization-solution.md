...
> 💡 Group Normalization 是 [Batch Normalization](https://leetgpu.com/challenges/batch-normalization) 和 [LayerNorm](../../medium/50_rms_normalization/leetgpu-rms-normalization-solution.md) 的折中方案——BatchNorm 的归约跨 `(N,H,W)` 维度（依赖 batch 大小），LayerNorm 跨 `(C,H,W)` 维度（每个样本独立），而 GroupNorm 跨 `(C/G, H, W)` 维度，**不受 batch size 影响**且**组内通道共享统计量**。它常用于小 batch 目标检测/分割（如 Mask R-CNN），也是扩散模型 U-Net 的标配归一化层。本题是练习"两遍 scan + shared memory reduction"的经典模板。
...
> ⚠️ GroupNorm 的核心难点和 [RMSNorm](../../medium/50_rms_normalization/leetgpu-rms-normalization-solution.md) 一样：**它是"归约 + 归一化"的组合**，且比 RMSNorm 多一次归约（mean + var 两次）。必须用**块内协作**——一个 block 的线程共同求 mean/var，再一起写输出。每 thread 独立扫整组会导致 `O(EPG²)` 重复读。
...
> 💡 GroupNorm 比 [RMSNorm](../../medium/50_rms_normalization/leetgpu-rms-normalization-solution.md) 多一次归约——RMSNorm 直接对 `x²` 求和（省掉 mean），而 GroupNorm 需要先求 mean 再求 var（variance 依赖 mean），这是 LayerNorm/GroupNorm/BatchNorm 共有的"两次归约"模式。
...
GroupNorm 需要 mean 和 var 两次统计量。直接复用 [Day 4 Reduction](../../medium/4_reduction/leetgpu-reduction-solution.md) 的 `warp_reduce_sum` + `block_reduce_sum` 模板：
...
> 💡 **关键洞察**：GroupNorm 的核心是"两次归约 + 一次归一化"——Pass 1 求 mean，Pass 2 用 mean 求 var（variance 依赖 mean），Pass 3 用 mean 和 var 归一化。这比 [RMSNorm](../../medium/50_rms_normalization/leetgpu-rms-normalization-solution.md) 的"一次归约"多一遍 HBM 读，是 LayerNorm/GroupNorm/BatchNorm 共有的代价。归约次数直接决定 global memory 读取遍数——三遍 scan 共读 X 三次，这是 memory-bound 的根源。

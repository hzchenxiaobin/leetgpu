...
> 💡 这道题是 **log-sum-exp（LSE）+ 两遍归约 + 跨 batch 平均**的综合练习。它与 [Softmax](../../medium/5_softmax/leetgpu-softmax-solution.md) 共享同一个"减 max 保数值稳定"的骨架，但目标不同：Softmax 输出整行概率分布，Cross Entropy 只需一个标量损失。这使 Cross Entropy 可以**跳过归一化**，直接在归约阶段融合 LSE 计算，减少一次全行扫描。
...
1. **减 max 保数值稳定**：与 [Softmax](../../medium/5_softmax/leetgpu-softmax-solution.md) 完全相同的技巧。$e^{z - m}$ 最大值为 $e^0 = 1$，彻底避免 overflow。
2. **warp shuffle 两级归约**：warp 内用 `__shfl_down_sync` 树形归约 → warp 0 用 shared memory 做 warp 间归约。与 [Reduction](../../medium/4_reduction/leetgpu-reduction-solution.md) 和 [Dot Product](../../medium/17_dot_product/leetgpu-dot-product-solution.md) 同一模板。

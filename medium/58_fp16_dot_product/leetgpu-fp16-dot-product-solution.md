...
> 💡 这道题是 [Dot Product（#17）](../../medium/17_dot_product/leetgpu-dot-product-solution.md) 的半精度变体。核心难点不在归约结构（与 FP32 版完全一致），而在于**精度保证**：FP16 只有 10 bit 尾数（约 3 位有效十进制），直接累加大数组会迅速丢失精度。标准做法是**读 half、转 FP32、FP32 累加、最后再转回 half**——即 `__half2float()` / `__float2half()` 两次类型转换。

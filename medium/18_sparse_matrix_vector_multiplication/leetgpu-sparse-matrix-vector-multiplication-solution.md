...
> 💡 这道题是 **CSR 格式 + SpMV（稀疏矩阵-向量乘）**的经典练习——HPC 领域最重要的 kernel 之一。平台传入稠密 `A` 与 `nnz`，意图明显：朴素做稠密 GEMV 会读 65% 的零（浪费带宽、做无谓乘法）；正解是**先把稠密 `A` 压缩成 CSR 三元组**，再用 SpMV 只遍历非零元。它与 [#22 GEMM](../../medium/22_gemm/leetgpu-gemm-solution.md) 的对比揭示了 GPU 编程的一个分水岭：稠密计算访存**规则合并**（coalesced），而稀疏计算访存**不规则间接**（gather via `col_idx`）——后者是另一类 memory-bound 优化的核心。

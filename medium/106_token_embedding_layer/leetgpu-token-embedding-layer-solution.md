...
> ⚠️ 正确做法：**一个 kernel 内完成 gather + 加 + LayerNorm**，中间的 `s[D]` 只活在寄存器/shared memory，永不落 HBM。这与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 消除 S/P 物化同理——"融合消除中间矩阵"。

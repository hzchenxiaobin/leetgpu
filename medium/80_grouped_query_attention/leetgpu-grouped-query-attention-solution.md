...
2. **online softmax 三公式**（与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 完全一致）：遍历 `seq_len` 个 key，用 `m/l/o` 增量更新，`S=QK^T` 和 `P=softmax(S)` 永不落 HBM。
...
3. **FlashAttention tiling**：当 `seq_len` 很大时，把 KV 分块载入 shared，Q tile 常驻，把 HBM IO 从 `O(nq × S² × d)` 降到 `O(S × d)` 级别（与 [Week4 FlashAttention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 同理）。

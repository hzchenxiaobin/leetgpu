...
> 💡 这道题是 [Sparse Matrix-Vector Multiplication (SpMV)](../../medium/18_sparse_matrix_vector_multiplication/leetgpu-sparse-matrix-vector-multiplication-solution.md) 的矩阵版进阶。SpMV 是 $A \times v$（$v$ 是向量），SpMM 是 $A \times B$（$B$ 是矩阵）。核心区别：SpMV 每个非零元只乘一个标量，SpMM 每个非零元要乘 $B$ 的一整行（$K$ 个元素）并累加。这使得 SpMM 的**算术强度**（FLOP/byte）比 SpMV 高 $\sim K$ 倍，从纯 memory-bound 逐步向 compute-bound 移动。

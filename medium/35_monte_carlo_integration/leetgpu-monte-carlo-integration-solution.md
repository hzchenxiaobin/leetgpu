...
> 💡 这道题本质是 **大规模 sum reduction**——核心计算是 $\sum y_i$，与 [#4 Reduction](../../medium/4_reduction/leetgpu-reduction-solution.md) 完全同构。唯一的区别是归约结果需要再除以 $n$ 并乘以 $(b-a)$。n=10M 的归约是典型的 **memory-bound** kernel（算术强度仅 ~0.25 FLOP/B），优化方向与 Reduction 相同：grid-stride loop + warp shuffle + 两级归约。

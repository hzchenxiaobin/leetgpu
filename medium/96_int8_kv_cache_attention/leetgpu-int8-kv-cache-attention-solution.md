...
> ⚠️ 正确做法是**把反量化流式地融在 attention kernel 里**：从 HBM 读 int8 K/V → 寄存器里乘 scale 成 fp32 → 立刻用于点积/加权 → 丢弃。fp32 K/V 永不落 HBM。这就是"kernel fusion 消除中间矩阵物化"的又一次应用（与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 消除 S/P 物化同源）。
...
2. **online softmax 三公式**（与 [Week4 Softmax Attention](../../medium/6_softmax_attention/leetgpu-softmax-attention-solution.md) 完全一致）：遍历 `L` 个 token，用 `m/l/o` 增量更新，`S=QK^T` 和 `P=softmax(S)` 永不落 HBM。
